...
| Profile clear endpoint | `src/server.js`, `src/browser-manager.js` | `POST /_broker/profiles/clear` deletes a named broker-managed profile when it is not active. | Destructive remote API; safe only on localhost/trusted SSH and rejects active profile dirs. |
